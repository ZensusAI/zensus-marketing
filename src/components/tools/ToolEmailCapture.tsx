...
export function ToolEmailCapture({ tool, inputs }: ToolEmailCaptureProps) {
  ...
  const validateEmail = () => {
    ...
    // Marketing consent deliberately does NOT gate submission. Submitting this
    // form IS the request for the breakdown, so delivering it is transactional
    // and needs no separate opt-in. Bundling delivery and marketing into one
    // required tick makes the marketing consent a condition of a service the
    // user asked for, which GDPR Art. 7(4) does not treat as freely given.
    return "";
  };
    ...
    <div className="rounded-2xl border border-border bg-card/50 p-6 mb-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
        ...
        <label className="flex items-start gap-2 text-sm text-muted-foreground">
          ...
          />
          <span>
            Also send me occasional Zensus updates. Optional, and you can
